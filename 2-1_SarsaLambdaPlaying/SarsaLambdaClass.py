...
os.chdir('2-1_SarsaLambdaPlaying/mem_and_log/')
...
class SarsaLambda:
    def __init__(self, actions, learning_rate=0.01, reward_decay=0.8, e_greedy=0,epsilonAdd = 0.1,lambda_=0.9,save_mem_round=100,train_mode=1):
    ...
    def __save_get_memorize(self,read_flag):
        exits_flag = os.path.exists(Memory) and os.path.exists(Memory2)
        # True: read, False: save
        if read_flag:
            if exits_flag:
                self.q_table = pd.read_csv(Memory,index_col=0,header=0)
                self.eligibility_trace = pd.read_csv(Memory2,index_col=0,header=0)
                # self.q_table.index = [str(a) for a in  self.q_table.index ]
                print('~~~Reading memory!~~~')
        else:
            self.q_table.to_csv(Memory)
            self.eligibility_trace.to_csv(Memory2)
            print('~~~Saving memory!~~~')
    ...
    def learn(self, s, a,a_,r, s_,done):
        ...
        loss = q_target - q_predict
        abs_loss =abs(loss)
        self.step_loss += abs_loss
        self.steps += 1

        self.eligibility_trace.loc[new_s,:] *= 0
        self.eligibility_trace.loc[new_s,a]  = 1

        self.q_table += self.lr*loss*self.eligibility_trace
    ...
    def check_state_exist(self, state):
        ...
        if news not in self.q_table.index:
            ...
            self.q_table = self.q_table.append(temp)
            self.eligibility_trace = self.eligibility_trace.append(temp)
        return news